...
# and ask every existing user to accept again. Not material: the change
# touches only the website waitlist, which is never linked to anything in the app.
material: false
changeSummary: 'The website waitlist now also carries beta invitations, and we see whether our emails to it are opened. Nothing in the app changes.'
---
...
- **If you join the waitlist on our website, we hold your email address** — only because you typed it in, only to invite you to the beta and tell you when Lysning is live, and you can unsubscribe from any message we send. We also see whether those emails are opened and which links in them are clicked. It is never linked to anything in the app.
...
Our website at lysning.app offers an optional waitlist. If you choose to submit it, we receive **your email address** — no name, no financial data, and nothing connected to your use of the app. When we email you, we also see whether the email was opened and which links in it were clicked, using a tracking pixel and redirected links. That tells us which messages are worth sending; it is never used for advertising and never shared. There is no waitlist in the app, and submitting one is never required to use Lysning.

The form is hosted by **Buttondown** (Buttondown LLC, United States), who store the address on our behalf under a data processing agreement. We use it for two things only: to invite you to the beta, and to tell you when Lysning is publicly available. Every message includes a one-click unsubscribe, and we delete the list once the launch announcement has been sent (see Section 7).
...
- **Waitlist consent** (UK GDPR Art. 6(1)(a); PECR regs. 6 and 22): we email you only because you asked us to, and the form tells you before you submit it that we see whether our emails are opened. Unsubscribe via the link in any message, or by emailing **privacy@lysning.app** — either removes you from the list entirely.
...
- **Buttondown** (website waitlist; stores the email address you submit, and whether our emails to you were opened and their links clicked)

## 6. International transfers

Analytics data is stored by PostHog in the EU (covered by the UK adequacy decision for the EEA). Crash data may be processed by Sentry in the United States. Update checks are handled by Expo in the United States. Waitlist email addresses are stored by Buttondown in the United States; the transfer relies on Standard Contractual Clauses with the UK Addendum, as set out in Buttondown's data processing agreement. Website analytics are processed by Umami in the United States on the same basis, as set out in Umami's data processing agreement.
...
- **Waitlist email addresses,** and the open and click records that go with them: held until the launch announcement is sent, then deleted within 30 days — or immediately, whenever you unsubscribe or ask us to remove you.
